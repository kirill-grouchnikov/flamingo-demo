...
public class RibbonGalleryCheck extends JRibbonFrame {
    ...
    private static class TopLeftDecoration implements DecoratedResizableIcon.IconDecorator {
        ...
        @Override
        public void paintIconDecoration(Component c, Graphics g, int x, int y, int width,
                int height) {
            g.setColor(Color.blue.darker());
            g.drawString("" + number, x + 2, y + g.getFontMetrics().getAscent());
        }
    }
    ...
    private static class BottomRightDecoration implements DecoratedResizableIcon.IconDecorator {
        ...
        public void paintIconDecoration(Component c, Graphics g, int x, int y, int width,
            ...
            g.drawString("" + number, x + width - g.getFontMetrics().stringWidth("" + number) - 2,
    ...
    public RibbonGalleryCheck() {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 10; j++) {
                this.buttons[i][j] = new JCommandToggleButton("Button " + i + ":" + j,
                        new DecoratedResizableIcon(
                                new DisabledResizableIcon(RibbonElementPriority.TOP, 32, 32),
                                new TopLeftDecoration(i), new BottomRightDecoration(j)));
            }
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            galleryButtons.add(new StringValuePair<List<JCommandToggleButton>>("Group " + i,
                    galleryButtonsList));
        }

        this.ribbonBand.addRibbonGallery(GALLERY_NAME, galleryButtons, visibleButtonCounts, 6, 4,
        ...
        addToGroup0.addActionListener((ActionEvent e) -> {
            JCommandToggleButton toAdd = new JCommandToggleButton("Button 0 new",
                    new DecoratedResizableIcon(
                            new DisabledResizableIcon(RibbonElementPriority.TOP, 32, 32),
                            new TopLeftDecoration(0), new BottomRightDecoration(99)));
            ribbonBand.addRibbonGalleryButtons(GALLERY_NAME, "Group 0", toAdd);
        });
        controlPanel.add(addToGroup0);

        JButton addToGroup2 = new JButton("Add two to 2");
        addToGroup2.addActionListener((ActionEvent e) -> {
            JCommandToggleButton toAdd = new JCommandToggleButton("Button 2 new",
                    new DecoratedResizableIcon(
                            new DisabledResizableIcon(RibbonElementPriority.TOP, 32, 32),
                            new TopLeftDecoration(2), new BottomRightDecoration(99)));
            JCommandToggleButton toAdd2 = new JCommandToggleButton("Button 2 new 2",
                    new DecoratedResizableIcon(
                            new DisabledResizableIcon(RibbonElementPriority.TOP, 32, 32),
                            new TopLeftDecoration(2), new BottomRightDecoration(66)));
            ribbonBand.addRibbonGalleryButtons(GALLERY_NAME, "Group 2", toAdd, toAdd2);
        });
